...
class Cleanup(threading.Thread):
    ...
    def __init__(self):
        ...
        self.NoneGroup = config.NoneGroup
        self.quit = False
    ...
    def run(self):
        ...
        while not self.quit:
            ...
            num_vm_busy = OpenstackCluster.num_vms_by_group_activity(vms=vms_new,group=self.NoneGroup,activity=ACTIVE_SET)
            print "num_vm_busy"
            print num_vm_busy

            num_vm_idle = OpenstackCluster.num_vms_by_group_activity(vms=vms_new,group=self.NoneGroup,activity=INACTIVE_SET)
            print "num_vm_idle"
            print num_vm_idle
            ...
            if (num_vm_to_destroy > 0):
                try:
                    OpenstackCluster.vm_destroy_by_Group_JobActivity(count=num_vm_to_destroy,group=self.NoneGroup,activity=INACTIVE_SET,vms=vms_new)
                except Exception as e:
            ...
            while (not self.quit) and self.sleep_tics > 0:
                time.sleep(1)
                self.sleep_tics -= 1
