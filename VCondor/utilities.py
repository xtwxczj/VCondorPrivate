...
## utilities.py - utility functions not specific to VRManager
...
def setHostnameByIp(ip='',):
    ...
    try:
        ...
        _tempname1 = IpList[2]
        if(3-len(IpList[2])>0):
            for i in range(0,3-len(IpList[2])):
                _tempname1 ='0'+_tempname1

        _tempname2 = IpList[3]
        if(3-len(IpList[3])>0):
            for i in range(0,3-len(IpList[3])):
                _tempname2 = '0' + _tempname2

        return 'vm'+ _tempname1 + _tempname2
    except Exception, e:
        print e
        return 0
