...
class OpenStackCluster(cluster_tools.ICluster,cluster_tools.VM):
    ...
    ERROR = 1
    #DEFAULT_INSTANCE_FLAVOR = config.default_VMInstanceFlavor if config.default_VMInstanceFlavor else "m1.small"
    #DEFAULT_INSTANCE_FLAVOR_LIST = _attr_list_to_dict(config.default_VMInstanceFlavorList)
    VM_STATES = {
            "BUILD" : "Starting",
            "ACTIVE" : "Running",
            "SHUTOFF" : "Shutdown",
    ...
    def __init__(self,username,password,tenant_id,auth_url,name='openstack cluster',):
        ...
        #self.tenant_name = tenant_name if tenant_name else "admin"
    ...
    def vm_create(self,vm_name="", resource_group_type="", group="", imageId="",instance_flavorId="",
        ...
        try:
            ...
            print "Unable to import novaclient images!"
            sys.exit(1)
        
        try:
            imagemanager = images.ImageManager(self)
            imageobj = imagemaneger.get(self,imageId)
            ...
            log.exception("Unable to fetch image via uuid: %s %s" % (imageId e))
        ...
        try:
            ...
            try:
                ...
                # self.failed_image_set.add(imageId)
                return
        ...
        try:
            ...
            print "Unable to import novaclient flavors!"
            sys.exit(1)

        try:
            flavormanager = flavors.FlavorManager(self)
            flavorobj = flavormanager.get(self,instance_flavorId)
        except Exception as e:
            log.exception("Unable to get flavor via flavor id: %s %s" % (instance_flavorId e))
        """
        ...
        try:   
            ...
            try:
                ...
                print ex
                self.logger.error("Exception occurred trying to get flavor by uuid: %s" % instance_flavorId)
                return
        # self.flavor_set.add(flavorobj)
        ...
        if vm_networkassoc:
            try:
                ...
                if network:
                    ...
                    print "Unable to find network named: %s on %s" % (vm_networkassoc, self.name)
